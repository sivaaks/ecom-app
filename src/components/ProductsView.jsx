...
import { Grid,Stack,Typography,Slider, Button, Checkbox, Rating, FormGroup, FormControlLabel } from '@mui/material';
...
export default function ProductsView(){
    ...
    return(
        ...
        <Grid sx={{backgroundColor:'#f0f2f5'}}>
            <Appbar/>
            <Stack direction="row">
                <Stack margin={1} spacing={1} minWidth={200} padding={1} backgroundColor='white'>
                    <Typography sx={{fontWeight:'bold'}}>Filters</Typography>
                    <hr/>
                    <Typography>DELIVERY</Typography>
                        <FormGroup>
                            <FormControlLabel onChange={freeDeliveryChecked} name="freeDelivery" control={<Checkbox></Checkbox>} label="Free delivery"></FormControlLabel>
                        </FormGroup>
                    <hr/>
                    <Typography>ASSURED</Typography>
                        <FormGroup>
                            <FormControlLabel onChange={assuredChecked} name="freeDelivery" control={<Checkbox sx={{paddingTop:'1px'}}></Checkbox>} label={<img alt="fassured" height="21px" width="77px" src="/fassured.png"></img>}></FormControlLabel>
                        </FormGroup>
                    <hr/>
                    ...
                    <Slider
                        ...
                        disableSwap
                    />
                    <Typography>Min:{price[0]} to Max:{price[1]}</Typography>
                    <Stack direction="row">
                        <Button onClick={filterDataByPrice}>Filter</Button>
                        <Button onClick={resetOriginalData} color="error">Reset</Button>
                    </Stack>
                    <hr/>
                ...
                <Stack margin={1} spacing={1} backgroundColor="#fff" padding={1} width="100%">
                    <Typography fontWeight="bold">Products</Typography>
                    <Stack direction="row" alignItems="center">
                        <Typography fontSize="14px" fontWeight="bold">Sort by</Typography>
                        {/* <Button size="14px">Popularity</Button> */}
                        <Button onClick={()=>{sortData(ASC)}} size="14px">Price-Low to high</Button>
                        <Button onClick={()=>{sortData(DESC)}} size="14px">Price-High to low</Button>
                    ...
                    {products.map((product,index)=>{
                       return(
                       <>
                       <Stack onClick={()=>{history.push(`/view/${product._id}`)}} padding={3} direction="row" spacing={3} alignItems="top-center" key={index}>
                        <Grid item md={3} xs={12}>
                            <Stack justifyContent="center" id="img">
                                <img alt="prod-img" width="200px" src={product.img?product.img:'/prod-default.jpg'}></img>
                            </Stack>
                        </Grid>
                        ...
                        <Grid item md={3} xs={12}>
                            <Stack id="price">
                                <Stack direction="row" spacing={2} alignItems="center">
                                    <Typography width="auto" fontSize="28px" fontWeight="bold">₹{product.salePrice}</Typography>
                                    {product.assured==="yes"?<img alt="fassured" height="21px" width="77px" src="/fassured.png"></img>:<></>}
                                </Stack>
                                <Stack direction="row" spacing={1}>
                                    <Typography sx={{textDecoration:'line-through',fontSize:'16px',color:'gray' }}>₹{product.price}</Typography>
                                    <Typography sx={{color:'green'}}>{calculateDiscountPercentage(product.price,product.salePrice)}% off</Typography>
                            </Stack>
                            <Typography>{product.deliveryCharge>0?`Delivery charge ₹${product.deliveryCharge}`:'Free delivery'}</Typography>
                            </Stack>
                        </Grid>
                    </Stack>
                    <hr/>
